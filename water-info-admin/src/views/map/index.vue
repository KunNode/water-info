...
import { getLatestObservation } from '@/api/observation'
...
async function refreshMap() {
  try {
    ...
    await Promise.all(
      filteredStations.value.map(async (station) => {
        ...
        try {
          const latest = await getLatestObservation(station.id, metric)
          markers.push({
            station,
            latestValue: latest.data?.value ?? null,
